...
TMP_CACHE="{CACHE%*.m3u}.txt"
find "${STORE_PREFIX}" -iregex '^.*\.\(\(mp3\)\|\(flac\)\|\(ogg\)|\(ogg\)|\(wma\)\)$' | grep -E albums/ > $TMP_CACHE # albums only
mv $TMP_CACHE $CACHE
rm $TMP_CACHE
...
#find "${STORE_PREFIX}" -iregex '^.*\.\(\(mp3\)\|\(flac\)\|\(ogg\)|\(ogg\)|\(wma\)\)$' | grep -E singles/ > "${CACHE}" # albums only
# TODO
