...
cat "$CACHE_NAME" | egrep --color=always $IGNORE_CASE "$SEARCH_TERM1" | tee ~/.music_shell/queue
cat "$CACHE_NAME" | egrep --color=always $IGNORE_CASE "$SEARCH_TERM2" | tee -a ~/.music_shell/queue
